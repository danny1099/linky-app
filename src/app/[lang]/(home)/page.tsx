...
export default async function Home() {
  ...
  return (
    <section className="size-full bg-background px-4 py-2 md:px-20">
      <section className="flex size-full flex-col items-center py-20">
        <div className="mt-5 flex flex-col items-center space-y-2 text-pretty text-center">
          <Heading type="h1" className="text-2xl font-bold md:text-4xl">
            {t('title')}
          </Heading>
        ...
        <div className="mt-10 flex w-full flex-col items-center justify-center gap-3 p-2 md:w-1/2 md:flex-row">
          <Input type="text" placeholder={t('input-placeholder')} className="bg-accent max-sm:w-full" />
          <Button icon="send" className="max-sm:w-48">
            {t('button-text')}
          </Button>
        </div>
      </section>
